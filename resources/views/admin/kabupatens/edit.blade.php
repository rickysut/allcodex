    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.kabupatens.update", [$kabupaten->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kd_prop') ? 'is-invalid' : '' }}" name="kd_prop_id" id="kd_prop_id" required>
                    @foreach($kd_props as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kd_prop_id') ? old('kd_prop_id') : $kabupaten->kd_prop->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kd_kab') ? 'is-invalid' : '' }}" type="number" name="kd_kab" id="kd_kab" value="{{ old('kd_kab', $kabupaten->kd_kab) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama_kab') ? 'is-invalid' : '' }}" type="text" name="nama_kab" id="nama_kab" value="{{ old('nama_kab', $kabupaten->nama_kab) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lat') ? 'is-invalid' : '' }}" type="number" name="lat" id="lat" value="{{ old('lat', $kabupaten->lat) }}" step="0.0000000001">
                @if($errors->has('lat'))
                    <div class="invalid-feedback">
                        {{ $errors->first('lat') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lng') ? 'is-invalid' : '' }}" type="number" name="lng" id="lng" value="{{ old('lng', $kabupaten->lng) }}" step="0.0000000001">
                @if($errors->has('lng'))
                    <div class="invalid-feedback">
                        {{ $errors->first('lng') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tz') ? 'is-invalid' : '' }}" type="number" name="tz" id="tz" value="{{ old('tz', $kabupaten->tz) }}" step="1">
                @if($errors->has('tz'))
                    <div class="invalid-feedback">
                        {{ $errors->first('tz') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('path') ? 'is-invalid' : '' }}" type="text" name="path" id="path" value="{{ old('path', $kabupaten->path) }}">
                @if($errors->has('path'))
                    <div class="invalid-feedback">
                        {{ $errors->first('path') }}
                    </div>
